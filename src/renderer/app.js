const state = {
  ...
  update: null,
  logFilter: "all",
  logs: { all: [], postgres: [], backend: [], frontend: [], tasks: [] }
};

const labels = { postgres: "PostgreSQL", backend: "Spring Boot API", frontend: "Angular Frontend" };
...
function renderDashboard() {
  ...
  el("metricFailures").textContent=snap.metrics?.failuresToday??0;
  ...
  el("serviceGrid").innerHTML=services.map(service=>{
    ...
    if(h?.supported===true){hText=h.ok?`HTTP ${h.status} OK`:`HTTP ${h.status||"DOWN"}`;hClass=h.ok?"good":"bad";}
    else if(h?.supported===false&&service.portOpen){hText="Port OK · health endpoint absent";hClass="good";}
    const build=service.name==="frontend"?` · build ${service.buildStatus||"unknown"}`:"";
    return `<article class="service-card">
      <div class="service-head"><div><div class="service-name">${esc(labels[service.name]||service.name)}</div><div class="service-sub">${esc(cfg.startCommand||"")}</div></div><span class="badge ${esc(service.status)}">${esc(statusLabels[service.status]||service.status)}</span></div>
...
function renderDiagnostics(result){const host=el("diagnosticsResult");host.className="diagnostics-grid";host.innerHTML=result.checks.map(c=>`<div class="diagnostic-card ${c.neutral?"neutral":c.ok?"ok":"bad"}"><span class="diagnostic-icon"></span><div><div class="diagnostic-label">${esc(c.label)}</div><div class="diagnostic-detail">${esc(c.detail)}</div></div></div>`).join("");}

function hydrateSettings(){const c=state.config;if(!c)return;el("cfgWorkspace").value=c.workspace||"";el("cfgPostgresPort").value=c.services?.postgres?.port||5432;el("cfgBackendPort").value=c.services?.backend?.port||8080;el("cfgFrontendPort").value=c.services?.frontend?.port||4200;el("cfgApplicationUrl").value=c.urls?.application||"";el("cfgBackendUrl").value=c.urls?.backend||"";el("cfgSwaggerUrl").value=c.urls?.swagger||"";el("cfgPgadminUrl").value=c.urls?.pgadmin||"";el("cfgUpdateUrl").value=c.updates?.genericUrl||"";el("cfgAutoUpdate").checked=c.updates?.autoCheck!==false;el("cfgAutoDownload").checked=c.updates?.autoDownload!==false;el("cfgNotifications").checked=c.notifications?.enabled!==false;el("cfgLaunchAtLogin").checked=Boolean(c.windows?.launchAtLogin);el("cfgAllowReset").checked=Boolean(c.safety?.allowDatabaseReset);el("resetDatabase").disabled=!c.safety?.allowDatabaseReset;el("openPgadmin").disabled=!c.urls?.pgadmin;renderProfiles();}
...
async function loadLogs(){for(const n of ["all","postgres","backend","frontend","tasks"])state.logs[n]=await window.men.getLogs(n);renderLogs();}
...
el("openApp").onclick=()=>window.men.openApplication();el("openWorkspace").onclick=()=>window.men.openWorkspace();el("openLogsFolder").onclick=()=>window.men.openLogDirectory();el("openSwagger").onclick=()=>window.men.openSwagger();el("openBackend").onclick=()=>window.men.openBackend();el("openPgadmin").onclick=()=>window.men.openPgadmin();
el("refreshOverview").onclick=refreshOverview;el("refreshDatabase").onclick=refreshOverview;
...
(async()=>{state.config=await window.men.getConfig();hydrateSettings();await Promise.all([refreshSnapshot(),loadLogs(),refreshTasks(),refreshSession(),refreshUpdate()]);await refreshOverview();setInterval(()=>refreshOverview().catch(()=>{}),10000);})();
