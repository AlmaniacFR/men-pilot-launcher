...
contextBridge.exposeInMainWorld("pilotage",{
  telemetry:(hours=1)=>ipcRenderer.invoke("pilotage:telemetry",hours),
  createDiagnosticReport:()=>ipcRenderer.invoke("pilotage:report-create"),
  restorePoints:()=>ipcRenderer.invoke("pilotage:restore-list"),
  createRestorePoint:label=>ipcRenderer.invoke("pilotage:restore-create",label),
  restorePoint:id=>ipcRenderer.invoke("pilotage:restore-run",id),
  readiness:()=>ipcRenderer.invoke("pilotage:readiness"),
  createClaudeContext:()=>ipcRenderer.invoke("pilotage:claude-create"),
  revealFile:file=>ipcRenderer.invoke("pilotage:open-file",file)
});

window.addEventListener("DOMContentLoaded",()=>{
  for(const href of ["./ux-v31.css","./pilotage-v32.css"]){const css=document.createElement("link");css.rel="stylesheet";css.href=href;document.head.appendChild(css);}
  for(const src of ["./ux-v31.js","./pilotage-v32.js"]){const script=document.createElement("script");script.src=src;script.defer=true;document.body.appendChild(script);}
},{once:true});
